import { iChart, iObservable } from "../../../../env/types";
...
export class Chart implements iChart {
  ...
  constructor(data: Array<number>, headerData: Array<string>, size: size, chartType: string = "line") {
    this.chartData = data;
  ...
  setMarkCount(count: number): void {
    if (typeof count === "number" && count > 2) {
      ...
      this.error$.next("Недопустимое значение счетчика. Укажите число, большее или равное 2.");
    }
  }
  ...
  setColumnBorder(borderData: string): void {
    ...
    if (this.chartColumnContainer) {
      ...
      this.error$.next("Неверный формат данных границы. Пожалуйста, укажите строковый формат.");
    }
